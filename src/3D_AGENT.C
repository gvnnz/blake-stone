...
void ComputeAvailWeapons(void)
{
    ...
    if (gamestate.ammo)
    {
        if (gamestate.ammo >= BFG_ENERGY_USE)
            gamestate.useable_weapons = (1 << wp_bfg_cannon) | (1 << wp_grenade) | (1 << wp_ion_cannon) | (1 << wp_burst_rifle) | (1 << wp_pistol) | (1 << wp_autocharge);
        else if (gamestate.ammo >= GRENADE_ENERGY_USE)
            gamestate.useable_weapons = (1 << wp_grenade) | (1 << wp_ion_cannon) | (1 << wp_burst_rifle) | (1 << wp_pistol) | (1 << wp_autocharge);
        else
            gamestate.useable_weapons = (1 << wp_ion_cannon) | (1 << wp_burst_rifle) | (1 << wp_pistol) | (1 << wp_autocharge);
    }
    else
...
{
#define FC_EMBED_COLOR(ColorCodes)               \
    {                                            \
        _fstrncpy(&pbuffer[pos], ColorCodes, 5); \
        pos += 5;                                \
    }
#define FC_NORM_COLOR() FC_EMBED_COLOR("^FC57")
    ...
    for (y = 0; y < 64; y++)
    {
        ...
        for (x = 0; x < 64; x++)
        {
            ...
            else if (((unsigned)actor && (unsigned)actor < 108) || // 108 == LAST WALL TILE
