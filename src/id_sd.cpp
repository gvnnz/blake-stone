...
SDL_CheckSB(int port)
{
    ...
    asm mov       dx, alFMStatus // Wait >4usec
        asm in    al,
        dx asm in al, dx asm in al, dx asm in al, dx asm in al, dx asm in al, dx asm in al, dx asm in al, dx asm in al, dx

                                                                                                                            sbOut(sbReset, false); // Turn off sb DSP reset
    asm mov                                                                                                                 dx, alFMStatus         // Wait >100usec
        asm mov                                                                                                             cx,
        100 usecloop : asm in al, dx asm loop usecloop
...
SDL_SSService(void)
{
    ...
    while (ssSample)
    {
        asm mov         dx, [ssStatus] // Check to see if FIFO is currently empty
            asm in      al,
            dx asm test al, 0x40 asm jnz done // Nope - don't push any more data out
        ...
        asm mov         dx, [ssData] // Pump the value out
            ...
            asm mov dx,
            [ssControl] // Pulse printer select
            asm mov         al,
            [ssOff] asm out dx, al asm push ax asm pop ax asm mov al, [ssOn] asm out dx, al
...
SDL_CheckSS(void)
{
    ...
    asm mov         dx, [ssStatus] // Check to see if FIFO is currently empty
        asm in      al,
        dx asm test al, 0x40 asm jnz checkdone // Nope - Sound Source not here

        asm mov cx,
        32 // Force FIFO overflow (FIFO is 16 bytes)
        outloop : asm mov dx,
                  [ssData] // Pump a neutral value out
                  asm mov      al,
                  0x80 asm out dx,
                  al
                  ...
                  asm mov                           al,
                  [ssOff] asm out                   dx,
                  al asm push ax asm pop ax asm mov al,
                  [ssOn] asm out                    dx,
                  al
                  ...
                  asm mov dx,
                  [ssStatus] // Is FIFO overflowed now?
                  asm in      al,
                  dx asm test al,
                  0x40 asm jz checkdone // Nope, still not - Sound Source not here
...
{
    if (
        (leftpos < 0) || (leftpos > 15) || (rightpos < 0) || (rightpos > 15) || ((leftpos == 15) && (rightpos == 15)))
...
#else
    asm mov dx, alFMStatus asm mov cx, 100 usecloop : asm in al, dx asm loop usecloop
#endif
            ...
            if (env)
            {
                ...
                while (*env)
                {
                    ...
                    case 'A':
                        ...
                        if (
                            (temp >= 0x210) && (temp <= 0x280) && (!(temp & 0x00f)))
                            port = (temp - 0x200) >> 4;
                        else
                    ...
                    case 'I':
                        ...
                        if (
                            (temp >= 0) && (temp <= 10) && (sbIntVectors[temp] != -1))
                        {
                            sbInterrupt = temp;
                            sbIntVec    = sbIntVectors[sbInterrupt];
                        }
...
bool SD_PlaySound(soundnames sound)
{
    ...
    if ((DigiMode != sds_Off) && (DigiMap[sound] != -1))
    {
        ...
        {
            asm pushf asm cli if (DigiPriority && !DigiNumber)
            {
                asm popf
            ...
            asm popf
                ...
                if (s->priority < DigiPriority) return (false);
