...
fixed FixedByFrac(fixed a, fixed b)
{
    ...
    asm mov si, [WORD PTR b + 2] // sign of result = sign of fraction
        ...
        asm mov              ax,
        [WORD PTR a] asm mov cx, [WORD PTR a + 2]

                                     asm or
                                     cx,
        cx asm jns                                     aok : // negative?
                         asm neg cx asm neg ax asm sbb cx,
                         0 asm xor si,
                         0x8000 // toggle sign of result
                         aok :
        ...
        //
        asm mov bx,
        [WORD PTR b] asm mul bx // fraction*fraction
        asm mov di,
        dx // di is low word of result
        asm mov ax,
        cx         //
        ...
        //
        asm test si,
        0x8000 // is the result negative?
        asm jz ansok : asm neg dx asm neg ax asm sbb dx,
                       0
...
void VGAClearScreen(void)
{
    ...
    asm mov dx, SC_INDEX asm mov ax, SC_MAPMASK + 15 * 256 // write through all planes
                                                      asm out dx,
        ax

        asm mov    dx,
        80 asm mov ax, [viewwidth] asm shr ax, 2 asm sub dx, ax // dx = 40-viewwidth/2

        asm mov             bx,
        [viewwidth] asm shr bx, 3 // bl = viewwidth/8
        asm mov             bh,
        BYTE PTR[viewheight] asm shr bh, 1

        asm mov             es,
        [screenseg] asm mov di, [bufferofs]

        asm mov              ax,
        [viewflags] asm test ax, GS_DRAW_CEILING asm jnz skiptop
        ...
        asm mov                  cl,
        bl asm rep stosw asm add di,
        dx asm dec bh asm jnz toploop
        ...
        asm jmp bottominit
            ...
            skiptop : asm mov               al,
                      ...
                      ax
                          ...
                          bottominit : asm mov              ax,
                                       [viewflags] asm test ax,
                                       GS_DRAW_FLOOR asm jnz exit_mofo

                                       asm mov bh,
                                       BYTE PTR[viewheight] asm shr bh,
                                       1 asm mov                    ax,
                                       [BottomColor]
...
void ThreeDRefresh(void)
{
    ...
    //
    asm mov ax, ds asm mov es, ax asm mov di, OFFSET spotvis asm xor ax, ax asm mov cx, 2048 // 64*64 / 2
                                                                                        asm rep stosw
...
#ifdef PAGEFLIP
    asm cli asm mov cx, [bufferofs] asm mov dx, 3d4h // CRTC address register
        asm mov al,
        0ch // start address high register
        asm out               dx,
        al asm inc dx asm mov al, ch asm out dx, al // set the high byte
                                                 asm sti
#endif
